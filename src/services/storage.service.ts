...
export class StorageService<B = unknown> {
  ...
  chromeStorageStateChanges$: Observable<B> = this._chromeStorageStateChanges$.asObservable().pipe(
    switchMap((res) => {
  ...
  constructor(@Inject(APP) private readonly app: TApplication) {
    chrome.storage.local.get((state) => {
      ...
      merge(
        this.chromeStorageOnChanged$.pipe(startWith({})).pipe(
          map((changes) => {
            const stateUpdate = Object.keys(changes).reduce((acc, key) => ({ ...acc, [key]: changes[key].newValue }), {});
            return { ...stateUpdate };
          }),
        ),
        this.chromeStorageStateInvalidate$.pipe(debounceTime(10)).pipe(skipWhileProcessingMapTo(this.chromeStorageLocalGet$) as never),
      )
        // @ts-ignore
  ...
  selectInstant$<T extends keyof B>(key: T): Observable<B[T]> {
    return this._selectRoot$().pipe(map((value) => get(value, key)));
  }

  private _selectRoot$(): Observable<B> {
    this.chromeStorageStateInvalidate$.next();
    return this.chromeStorageStateChanges$.pipe(first<B>(Boolean));
  }
}
